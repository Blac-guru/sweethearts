...
const verificationSchema = z.object({
  ...
  idType: z.enum(["National ID", "Passport"], { required_error: "ID type is required" }),
...
export default function VerificationPage() {
  ...
  const verificationMutation = useMutation({
    mutationFn: async (data: VerificationData) => {
      ...
      if (data.idFront && data.idFront.length) formData.append("idFront", data.idFront[0]);
      if (data.idBack && data.idBack.length) formData.append("idBack", data.idBack[0]);
      if (data.selfie && data.selfie.length) formData.append("selfie", data.selfie[0]);

      // 🔍 Backend can now use facial recognition API (e.g. AWS Rekognition, Face++ etc.)
      // to compare selfie and ID photo

      return apiRequest("POST", "/api/verify", formData, globalToken);
    },
    onSuccess: () => {
      toast({
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                      ...
                      <FormItem>
                        ...
                        <div className="border-2 border-dashed rounded-lg p-6 text-center">
                          ...
                          <p className="text-sm text-muted-foreground mb-2">
                            Upload a clear photo of the <strong>front side</strong> of your ID.
                          </p>
                          <Input
                            type="file"
                            accept="image/*"
                    ...
                    render={({ field: { onChange } }) => (
                      <FormItem>
                        ...
                        <div className="border-2 border-dashed rounded-lg p-6 text-center">
                          <FileUp className="w-8 h-8 text-muted-foreground mx-auto mb-2" />
                          <p className="text-sm text-muted-foreground mb-2">
                            Upload a clear photo of the <strong>back side</strong> of your ID.
                          </p>
                          <Input
                            type="file"
                            accept="image/*"
                    ...
                    render={({ field: { onChange } }) => (
                      <FormItem>
                        ...
                        <div className="border-2 border-dashed rounded-lg p-6 text-center">
                          <FileUp className="w-8 h-8 text-muted-foreground mx-auto mb-2" />
                          <p className="text-sm text-muted-foreground mb-2">
                            Upload a selfie of you holding your ID clearly visible.
                          </p>
                          <Input
                            type="file"
                            accept="image/*"
                ...
                <div className="bg-muted rounded-md p-4">
                  <p className="text-sm text-muted-foreground text-center">
                    🔒 Your ID and selfie will be securely stored and used only for verification.
                  </p>
                </div>
